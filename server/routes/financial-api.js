...
router.get('/statements', async (req, res) => {
  const rows = await dbAll('SELECT id, company_name, statement_type, period_label, is_consolidated, created_at, updated_at FROM financial_statements WHERE user_id = ? ORDER BY updated_at DESC', [req.user.id]);
  res.json(rows);
});

// 個別取得
router.get('/statements/:id', async (req, res) => {
  const row = await dbGet('SELECT * FROM financial_statements WHERE id = ? AND user_id = ?', [parseInt(req.params.id), req.user.id]);
...
router.post('/statements', async (req, res) => {
  ...
  const r = await dbRun(
    'INSERT INTO financial_statements (user_id, company_name, statement_type, period_label, period_months, unit, pl_data, bs_data, cf_data, adjustments, validation_errors, is_consolidated, subsidiaries) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [req.user.id, company_name || '', statement_type || 'standalone', period_label || '', period_months || 12, unit || 'thousand',
...
router.put('/statements/:id', async (req, res) => {
  const existing = await dbGet('SELECT id FROM financial_statements WHERE id = ? AND user_id = ?', [parseInt(req.params.id), req.user.id]);
...
router.delete('/statements/:id', async (req, res) => {
  await dbRun('DELETE FROM financial_statements WHERE id = ? AND user_id = ?', [parseInt(req.params.id), req.user.id]);
  res.json({ success: true });
});
...
router.get('/plans', async (req, res) => {
  const rows = await dbAll('SELECT id, version_name, company_name, plan_years, actual_periods, is_locked, created_at, updated_at FROM business_plans WHERE user_id = ? ORDER BY created_at DESC', [req.user.id]);
  res.json(rows);
});

// 個別取得
router.get('/plans/:id', async (req, res) => {
  const row = await dbGet('SELECT * FROM business_plans WHERE id = ? AND user_id = ?', [parseInt(req.params.id), req.user.id]);
...
router.post('/plans', async (req, res) => {
  ...
  const r = await dbRun(
    'INSERT INTO business_plans (user_id, version_name, company_name, plan_years, actual_periods, stress_factor, corporate_tax_rate, pl_plan, bs_plan, cf_plan, fixed_assets, debt_schedule, segment_details) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [req.user.id, version_name, company_name || '', plan_years || 10, actual_periods || 3,
...
router.put('/plans/:id', async (req, res) => {
  const existing = await dbGet('SELECT id, is_locked FROM business_plans WHERE id = ? AND user_id = ?', [parseInt(req.params.id), req.user.id]);
...
router.post('/plans/:id/lock', async (req, res) => {
  const existing = await dbGet('SELECT id FROM business_plans WHERE id = ? AND user_id = ?', [parseInt(req.params.id), req.user.id]);
  if (!existing) return res.status(404).json({ error: '事業計画が見つかりません' });
...
router.get('/ratings', async (req, res) => {
  const rows = await dbAll('SELECT id, company_name, debtor_category, adjusted_category, rating_date, created_at FROM credit_ratings WHERE user_id = ? ORDER BY created_at DESC', [req.user.id]);
  res.json(rows);
});

// 個別取得
router.get('/ratings/:id', async (req, res) => {
  const row = await dbGet('SELECT * FROM credit_ratings WHERE id = ? AND user_id = ?', [parseInt(req.params.id), req.user.id]);
  if (!row) return res.status(404).json({ error: '格付データが見つかりません' });
...
router.post('/ratings', async (req, res) => {
  const { company_name, quantitative_scores, qualitative_scores, real_bs_adj, real_pl_adj, operating_cf, repayment_years, debtor_category, personal_assets, adjusted_category, report_html } = req.body;
  const r = await dbRun(
    'INSERT INTO credit_ratings (user_id, company_name, quantitative_scores, qualitative_scores, real_bs_adj, real_pl_adj, operating_cf, repayment_years, debtor_category, personal_assets, adjusted_category, report_html) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
