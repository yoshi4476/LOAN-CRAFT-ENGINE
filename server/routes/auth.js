...
router.post('/register', (req, res) => {
  try {
    ...
    const exists = dbGet('SELECT id FROM users WHERE email = ? AND deleted_at IS NULL', [email]);
    ...
    const result = dbRun('INSERT INTO users (name, email, password_hash, role, plan, renewal_date) VALUES (?, ?, ?, ?, ?, ?)', [name, email, hash, role, 'Free', renewalDate]);

    const user = dbGet('SELECT * FROM users WHERE id = ?', [result.lastInsertRowid]);
    const token = generateToken(user);
    dbRun('INSERT INTO audit_logs (user_id, action, detail) VALUES (?, ?, ?)', [user.id, 'REGISTER', `新規登録: ${email}`]);
...
router.post('/login', (req, res) => {
  try {
    ...
    const user = dbGet('SELECT * FROM users WHERE email = ? AND deleted_at IS NULL', [email]);
    ...
    dbRun('UPDATE users SET last_login = datetime("now"), updated_at = datetime("now") WHERE id = ?', [user.id]);
    const token = generateToken(user);
    dbRun('INSERT INTO audit_logs (user_id, action) VALUES (?, ?)', [user.id, 'LOGIN']);
...
router.get('/me', authenticate, (req, res) => {
  const user = dbGet('SELECT id, name, email, role, plan, status, renewal_date, joined_at, last_login FROM users WHERE id = ?', [req.user.id]);
